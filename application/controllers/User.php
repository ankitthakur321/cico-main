...
class User extends MY_Controller {
    ...
    function cancelBooking()
    {
        ...
        {
            $msg = array('message' => 'Booking does not cancelled','class' => 'alert alert-danger');
            $this->session->set_flashdata('status',$msg);
            return redirect('user/profile');
        }
    }
    ...
    function deactivate()
    {
        ...
        $data['deactivation_reason']= $this->input->post('reason');
        $data['deactivationStatus']= 1;
        
        $success= $this->userModel->deactivateUser($u_id, $data);
        if($success)
        {
            $msg = array('message' => '<b>Your CICO account has been deactivated.</b><br>', 'text' => 'To reactivate your account, log in using your old phone number and password. <br><br> We hope you come back soon.','class' => 'alert alert-danger');
            $this->session->set_flashdata('deactive',$msg);
        ...
        {
            $msg = array('message' => 'Some Went Wrong','class' => 'alert alert-danger');
            $this->session->set_flashdata('status',$msg);
            return redirect('user/profile');
        }
    }
